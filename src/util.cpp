...
int nAnonymizeCopicoCAmount = 500;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Copico
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Copico
    // Mac: ~/Library/Application Support/Copico
    // Unix: ~/.Copico
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Copico";
#else
    fs::path pathRet;
...
void createConf()       //Automatic copico.conf generation
{
    srand(time(NULL));
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "copico.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "copicod.pid"));
