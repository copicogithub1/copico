...
{
    // Convert Copico:// to Copico:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Copico://"))
    {
        uri.replace(0, 12, "Copico:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Copico.lnk";
}
...
{
    return GetAutostartDir() / "Copico.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Copico-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Copico-qt [" + tr("command-line options") + "]                     " + "\n";
